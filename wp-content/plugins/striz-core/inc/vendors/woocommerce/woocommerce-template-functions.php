...
if (!function_exists('osf_cart_link')) {
    ...
    function osf_cart_link() {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            $items = '';
            $items .= '<a data-toggle="toggle" class="cart-contents header-button" href="' . esc_url(wc_get_cart_url()) . '" title="' . __("View your shopping cart", "striz-core") . '">';
...
if (!function_exists('osf_cart_count_text')) {
    ...
    function osf_cart_count_text() {
        if (!empty(WC()->cart) && WC()->cart instanceof WC_Cart) {
            return '<span class="count-text">' . wp_kses_data(_n("item", "items", WC()->cart->get_cart_contents_count(), "striz-core")) . '</span>';
        }
...
if (!function_exists('osf_handheld_footer_bar_search')) {
    ...
    function osf_handheld_footer_bar_search() {
        echo '<a href="">' . esc_attr__('Search', 'striz-core') . '</a>';
        osf_product_search();
    }
}
...
if (!function_exists('osf_handheld_footer_bar_cart_link')) {
    ...
    function osf_handheld_footer_bar_cart_link() {
        ...
        <a class="footer-cart-contents" href="<?php echo esc_url(wc_get_cart_url()); ?>"
           title="<?php esc_attr_e('View your shopping cart', 'striz-core'); ?>">
...
if (!function_exists('osf_handheld_footer_bar_account_link')) {
    ...
    function osf_handheld_footer_bar_account_link() {
        echo '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'striz-core') . '</a>';
    }
}
...
if (!function_exists('osf_woocommerce_order_review_heading')) {
    function osf_woocommerce_order_review_heading() {
        echo ' <h3 class="order_review_heading">' . esc_attr__('Your order', 'striz-core') . '</h3>';
    }
}
...
if (!function_exists('osf_woocommerce_get_product_label_stock')) {
    function osf_woocommerce_get_product_label_stock() {
        ...
        if ($product->get_stock_status() == 'outofstock') {
            echo '<span class="stock-label">' . esc_html__('Out Of Stock', 'striz-core') . '</span>';
        } else {
            echo '<span class="stock-label">' . esc_html__('In Stock', 'striz-core') . '</span>';
        }
    }
}
...
if (!function_exists('osf_woocommerce_set_register_text')) {
    function osf_woocommerce_set_register_text() {
        echo '<div class="user-text">' . __("Creating an account is quick and easy, and will allow you to move through our checkout quicker.", "striz-core") . '</div>';
    }
}
        ...
        if ($time_sale) {
            ...
            echo '<div class="time">
                    <div class="opal-countdown clearfix typo-quaternary"
                        ...
                        data-days="' . esc_html__("days", "striz-core") . '" 
                        data-hours="' . esc_html__("hours", "striz-core") . '"
                        data-minutes="' . esc_html__("mins", "striz-core") . '"
                        data-seconds="' . esc_html__("secs", "striz-core") . '"
                        data-Message="' . esc_html__('Expired', 'striz-core') . '"
...
if (!function_exists('osf_active_filters')) {
    function osf_active_filters() {
        ...
        echo '<span class="osf_active_filters_label">' . esc_html__('Active Filters: ', 'striz-core') . '</span>
    <a class="clear-all" href="' . esc_url($link_remove_all) . '">' . __('Clear Filters', 'striz-core') . '</a>
</div>';
    }
}
...
if (!function_exists('osf_woocommerce_deal_progress')) {
    function osf_woocommerce_deal_progress() {
        ...
        <div class="deal-sold">
            <span class="deal-text d-block"><span><?php esc_html_e('HURRY! ONLY', 'striz-core') ?></span> <span
                        class="c-primary"><?php echo esc_attr(trim($limit - $sold)) ?></span> <span><?php esc_html_e('LEFT IN STOCK.', 'striz-core') ?></span></span>
            <div>
...
function otf_woocommerce_recently_viewed_product() {
    if (get_theme_mod('otf_woocommerce_extra_enable_product_recently_viewed', false)) {
        ...
        if (!empty($_COOKIE['otf_woocommerce_recently_viewed'])) {
            ...
            echo '<h2 class="otf-woocommerce-recently-viewed">' . esc_html__('Your Recently Viewed', 'striz-core') . '</h2>';
...
function osf_woocommerce_pagination_args($args) {
    $args['prev_text'] = __('PREV', 'striz-core');
    $args['next_text'] = __('NEXT', 'striz-core');
    return $args;
}
...
if ( ! function_exists( 'wvs_taxonomy_meta_fields' ) ){
    function wvs_taxonomy_meta_fields( $field_id = false ) {
        ...
        $fields['color'] = array(
            array(
                'label' => esc_html__( 'Color', 'striz-core' ), // <label>
                'desc'  => esc_html__( 'Choose a color', 'striz-core' ), // description
        ...
        $fields['image'] = array(
            array(
                'label' => esc_html__( 'Image', 'striz-core' ), // <label>
                'desc'  => esc_html__( 'Choose an Image', 'striz-core' ), // description
