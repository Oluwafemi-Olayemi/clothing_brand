...
class Opal_Woocommerce_Extra {
    ...
    public function __construct() {
        ...
        include_once(XSTREET_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-product-brands.php');
        include_once(XSTREET_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-wc-widget-layered-nav.php');
    ...
    public function enqueue_scripts($hook) {
        if ($hook === 'post.php' && get_post_type() === 'product') {
            wp_enqueue_script('otf-woocommerce-admin', XSTREET_CORE_PLUGIN_URL . 'assets/js/woocommerce/admin.js', array('jquery'), XSTREET_CORE_VERSION, true);
        }
    }
    ...
    public function custom_taxonomy_brands() {
        ...
        $labels = array(
            'name'                       => esc_html__('Brands', 'striz-core'),
            'singular_name'              => esc_html__('Brands', 'striz-core'),
            'menu_name'                  => esc_html__('Brands', 'striz-core'),
            'all_items'                  => esc_html__('All Brands', 'striz-core'),
            'parent_item'                => esc_html__('Parent Brand', 'striz-core'),
            'parent_item_colon'          => esc_html__('Parent Brand:', 'striz-core'),
            'new_item_name'              => esc_html__('New Brand Name', 'striz-core'),
            'add_new_item'               => esc_html__('Add New Brands', 'striz-core'),
            'edit_item'                  => esc_html__('Edit Brand', 'striz-core'),
            'update_item'                => esc_html__('Update Brand', 'striz-core'),
            'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'striz-core'),
            'search_items'               => esc_html__('Search Brands', 'striz-core'),
            'add_or_remove_items'        => esc_html__('Add or remove Brands', 'striz-core'),
            'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'striz-core'),
        );
        $args = array(
            'labels'            => $labels,
            'hierarchical'      => true,
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'striz-core') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'striz-core')); ?>
            <input type="number" min="<?php echo $min; ?>" class="short" name="_deal_quantity" id="_deal_quantity"
                   value="<?php echo esc_attr($quantity) ?>">

            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts" style="clear:both;display:block;"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'bicomart-core', 'filaminimal-core', 'striz-core', 'striz-core', 'striz-core', 'striz-core'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
