...
class OSF_Widget_Product_Brands extends WC_Widget{
    ...
    public function __construct() {
        ...
        $this->widget_description = __( 'A list or dropdown of product brands.', 'striz-core' );
        $this->widget_id          = 'woocommerce_product_brands';
        $this->widget_name        = __( 'Product Brands', 'striz-core' );
        $this->settings           = array(
            'title'  => array(
                'type'  => 'text',
                'std'   => __( 'Product Brands', 'striz-core' ),
                'label' => __( 'Title', 'striz-core' ),
            ),
            'dropdown' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show as dropdown', 'striz-core' ),
            ),
            'show_logo' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show logo brand', 'striz-core' ),
            ),
            'count' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Show product counts', 'striz-core' ),
            ),
            'hierarchical' => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => __( 'Show hierarchy', 'striz-core' ),
            ),
            'show_children_only' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Only show children of the current brand', 'striz-core' ),
            ),
            'hide_empty' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => __( 'Hide empty brands', 'striz-core' ),
            ),
            'max_depth'  => array(
                'type'  => 'text',
                'std'   => '',
                'label' => __( 'Maximum depth', 'striz-core' ),
            ),
        );
    ...
    public function widget( $args, $instance ) {
        ...
        if ( $dropdown ) {
            ...
            include_once( XSTREET_CORE_PLUGIN_DIR . '/inc/vendors/woocommerce/class-product-brand-list-walker.php' );

            $list_args['walker']                     = new OTF_Product_Brand_List_Walker;
            $list_args['title_li']                   = '';
            $list_args['pad_counts']                 = 1;
            $list_args['show_option_none']           = __( 'No product brands exist.', 'striz-core' );
            ...
            echo '<input type="text" id="otf-product-brands-'. $id .'" onkeyup="Otf_search_product_brand();" placeholder="'. esc_attr__('Search for brand name...', 'striz-core').'" title="'.esc_attr__('Type in a name','striz-core').'">';
