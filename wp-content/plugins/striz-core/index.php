 ...
 * used, this is required. Version: 3.0.6 Author: WpOpal Author URI:  http://www.wpopal.com/ License: GNU/GPLv3
 * http://www.gnu.org/licenses/gpl-3.0.html
 */

final class XStreetCore {
    /**
     * @var XStreetCore
     */
    ...
    public function setup_constants() {
        if (!defined('XSTREET_CORE_VERSION')) {
            define('XSTREET_CORE_VERSION', '3.0.6');
        }

        if (!defined('XSTREET_CORE_PLUGIN_DIR')) {
            define('XSTREET_CORE_PLUGIN_DIR', plugin_dir_path(__FILE__));
        }

        if (!defined('XSTREET_CORE_PLUGIN_URL')) {
            define('XSTREET_CORE_PLUGIN_URL', plugin_dir_url(__FILE__));
        }

        if (!defined('XSTREET_CORE_PLUGIN_FILE')) {
            define('XSTREET_CORE_PLUGIN_FILE', __FILE__);
        }
    }
    ...
    private function load_textdomain() {
        $lang_dir      = dirname(plugin_basename(XSTREET_CORE_PLUGIN_FILE)) . '/languages/';
        $lang_dir      = apply_filters('osf_languages_directory', $lang_dir);
        $locale        = apply_filters('plugin_locale', get_locale(), 'striz-core');
        $mofile        = sprintf('%1$s-%2$s.mo', 'striz-core', $locale);
        $mofile_local  = $lang_dir . $mofile;
        $mofile_global = WP_LANG_DIR . '/striz-core/' . $mofile;

        if (file_exists($mofile_global)) {
            load_textdomain('striz-core', $mofile_global);
        } else {
            if (file_exists($mofile_local)) {
                load_textdomain('striz-core', $mofile_local);
            } else {
                load_plugin_textdomain('striz-core', false, $lang_dir);
            }
        }
    }
    ...
    public function plugin_update() {
        ...
        Puc_v4_Factory::buildUpdateChecker(
            'http://source.wpopal.com/striz/dummy_data/update-plugin.json',
            __FILE__,
            'striz-core'
        );
    }
    ...
    public static function getInstance() {
        if (!isset(self::$instance) && !(self::$instance instanceof XStreetCore)) {
            self::$instance = new XStreetCore();
        }
...
return XStreetCore::getInstance();
