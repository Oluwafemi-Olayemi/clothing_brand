...
if (!class_exists('osf_WooCommerce')) :
    ...
    class osf_WooCommerce {
        ...
        public function mobile_handheld_footer_bar() {
            $links = array(
                'my-account' => '<a class="my-accrount-footer" href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '">' . esc_attr__('My Account', 'striz-core') . '</a>',
                'search'     => '<a class="search-footer" href="">' . esc_attr__('Search', 'striz-core') . '</a><div class="site-search">' . get_search_form(false) . '</div>',
                'cart'       => ' <a class="footer-cart-contents" href="' . esc_url(wc_get_cart_url()) . '" 
                title="' . esc_attr__('View your shopping cart', 'striz-core') . '">  
 <span class="count">' . wp_kses_data(WC()->cart->get_cart_contents_count()) . '</span>
        ...
        public function label_tooltip() {
            ...
            echo '<div id="osf-woocommerce-cart">' . esc_html__('Add to cart', 'striz-core') . '</div>';
            echo '</div>';
        }
        ...
        public function added_to_cart_template() {
            $text = esc_html__('has been added to your cart', 'striz-core');
            echo <<<HTML
...
HTML;
        ...
        public function widgets_init() {
            register_sidebar(array(
                'name'          => __('WooCommerce Shop', 'striz-core'),
                'id'            => 'sidebar-woocommerce-shop',
                'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'striz-core'),
            ...
            register_sidebar(array(
                'name'          => __('WooCommerce Detail', 'striz-core'),
            ...
            register_sidebar(array(
                'name'          => __('WooCommerce Shop Filters', 'striz-core'),
        ...
        public function render_woocommerce_shop_canvas() {
            ...
            if (is_active_sidebar('sidebar-woocommerce-shop-filters')) {
                echo '<div id="opal-canvas-filter" class="opal-canvas-filter ' . $position . '"><span class="filter-close">' . esc_html__('CLOSE', 'striz-core') . '</span><div class="opal-canvas-filter-wrap">';
        ...
        public function render_button_shop_canvas() {
            ...
            if (is_active_sidebar('sidebar-woocommerce-shop-filters')) {
                echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__('Filter', 'striz-core') . '</button>';
            }
        }
            ...
            if (!empty($query)) {
                ...
                if (is_object($product_object)) {
                    ...
                    if (!empty($product_sku)) {
                        $product_sku_display = ' - ' . __('Sku', 'striz-core') . ': ' . $product_sku;
                    }

                    $product_title_display = '';
                    if (!empty($product_title)) {
                        $product_title_display = ' - ' . __('Title', 'striz-core') . ': ' . $product_title;
                    }

                    $product_id_display = __('Id', 'striz-core') . ': ' . $product_id;
        ...
        public function productCategoryAutocompleteRender($query) {
            ...
            $term_slug_display = '';
            if (!empty($term_slug)) {
                $term_slug_display = ' - ' . __('Sku', 'striz-core') . ': ' . $term_slug;
            }

            $term_title_display = '';
            if (!empty($term_title)) {
                $term_title_display = ' - ' . __('Title', 'striz-core') . ': ' . $term_title;
            }

            $term_id_display = __('Id', 'striz-core') . ': ' . $term_id;
        ...
        public function taxonomy_metaboxes() {
            ...
            $cmb_term = new_cmb2_box(array(
                'id'           => 'product_cat',
                'title'        => __('Product Metabox', 'striz-core'), // Doesn't output for term boxes
        ...
        public function product_video_custom_field() {
            ...
            $cmb    = new_cmb2_box(array(
                ...
                'title'        => esc_html__('Product Video Config', 'striz-core'),
            ...
            $cmb->add_field(array(
                'name' => __('Product video', 'striz-core'),
                'desc' => __('Supports video from youtube and vimeo.', 'striz-core'),
                'id'   => $prefix . 'video',
                'type' => 'oembed',
            ));

            $cmb->add_field(array(
                'name'         => __('Video Thumbnail', 'striz-core'),
        ...
        public function woocommerce_scripts() {
            ...
            wp_enqueue_style('slick', trailingslashit(XSTREET_CORE_PLUGIN_URL) . 'assets/css/slick.css');
            wp_enqueue_script('slick', trailingslashit(XSTREET_CORE_PLUGIN_URL) . 'assets/js/libs/slick.js', array(), false, true);
            wp_enqueue_script('osf-woocommerce-main', trailingslashit(XSTREET_CORE_PLUGIN_URL) . 'assets/js/woocommerce/main.js', array(
                'jquery',
                'wp-util'
            ), false, true);

            if (is_product()) {
                wp_enqueue_script('osf-woocommerce-single', trailingslashit(XSTREET_CORE_PLUGIN_URL) . 'assets/js/woocommerce/single.js', array(), false, true);
                if (get_theme_mod('osf_woocommerce_single_product_tab_style', 'tab') == 'accordion') {
                    wp_enqueue_script('osf-woocommerce-single-accordion', trailingslashit(XSTREET_CORE_PLUGIN_URL) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true);
                }
            }
        ...
        public function custom_comment_form($comment_form) {
            ...
            $comment_form['fields']        = array(
                'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__('Name', 'striz-core') . '&nbsp;<span class="required">*</span></label> ' .
                            '<input id="author" name="author" type="text" placeholder="' . esc_attr__("Name", "striz-core") . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" required /></p>',
                'email'  => '<p class="comment-form-email"><label for="email">' . esc_html__('Email', 'striz-core') . '&nbsp;<span class="required">*</span></label> ' .
                            '<input id="email" name="email" type="email" placeholder="' . esc_attr__("Email", "striz-core") . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" required /></p>',
            );
            $comment_form['comment_field'] = '<p class="comment-form-comment"><label for="comment">' . esc_html__('Your review', 'striz-core') . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required placeholder="' . esc_attr__("Your review", "striz-core") . '"></textarea></p>';
            return $comment_form;
        }
