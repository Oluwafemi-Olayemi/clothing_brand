        ...
        private function check_activate() {
            if (is_admin() && current_user_can('administrator')) {
                ...
                if (!$check) {
                    ...
                    update_option('_wooscp_button_single', '41');

                    update_option('woosw_button_position_archive', '0');
                    update_option('woosw_button_position_single', '41');
        ...
        public function add_hook() {
            ...
            if (function_exists('woosw_init')) {
                if (function_exists('woosq_init') && get_option('woosq_button_position') == "0") {
                    add_action('woocommerce_shop_loop_item_title', [$this, 'quickview_button'], 35);
                }

                if (function_exists('woosc_init')) {
                    add_action('woocommerce_shop_loop_item_title', [$this, 'compare_button'], 30);
                }
                add_filter('woosc_button_position_archive', '__return_false');
                add_filter('woosc_button_position_single', '__return_false');

                if (function_exists('woosw_init') && get_option('woosw_button_position_archive') == "0") {
                    add_action('woocommerce_shop_loop_item_title', [$this, 'wishlist_button'], 25);
                }
            }
        }
