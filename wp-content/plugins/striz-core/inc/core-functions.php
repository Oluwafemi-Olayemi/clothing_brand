...
function osf_get_google_fonts() {
    $content = file_get_contents(XSTREET_CORE_PLUGIN_DIR . 'webfonts.json');
    ...
    function osf_scrape_instagram($username, $slice = 9) {
        ...
        if (false === ($instagram = get_transient('otf-instagram-media-new-' . sanitize_title_with_dashes($username)))) {
            ...
            if (is_wp_error($remote))
                return new WP_Error('site_down', esc_html__('Unable to communicate with Instagram.', 'striz-core'));

            if (200 != wp_remote_retrieve_response_code($remote))
                return new WP_Error('invalid_response', esc_html__('Instagram did not return a 200.', 'striz-core'));
            ...
            if (!$insta_array)
                return new WP_Error('bad_json', esc_html__('Instagram has returned invalid data.', 'striz-core'));
            ...
            if (isset($insta_array['entry_data']['UserProfile'][0]['userMedia'])) {
                ...
                return new WP_Error('bad_json_2', esc_html__('Instagram has returned invalid data.', 'striz-core'));
            }


            if (!is_array($images))
                return new WP_Error('bad_array', esc_html__('Instagram has returned invalid data.', 'striz-core'));
            ...
            switch ($type) {
                ...
                case 'old_2':
                    foreach ($images as $image) {
                        ...
                        $caption = esc_html__('Instagram Image', 'striz-core');
        ...
        if (!empty($instagram)) {
            ...
            return new WP_Error('no_images', esc_html__('Instagram did not return any images.', 'striz-core'));
        }
    }
}
...
if (!function_exists('osf_esc_font_weight')) {
    function osf_esc_font_weight($weight) {
...
function time_elapsed_string($datetime, $full = false) {
    ...
    $string = array(
        'y' => __('year', 'striz-core'),
        'm' => __('month', 'striz-core'),
        'w' => __('week', 'striz-core'),
        'd' => __('day', 'striz-core'),
//        'h' => __('hour', 'striz-core'),
//        'i' => __('minute', 'striz-core'),
//        's' => __('second', 'striz-core'),
    );
    ...
    return $string ? implode(', ', $string) . __(' left', 'striz-core') : __('just now', 'striz-core');
}
